...
describe('Plans And Investments', function() {
    ...
    it('Displays topup payment form on click of top up button in product details page.', function() {
        app().loginAs(pensionUser());
		app().selectPlanToViewInvestments();
        app().clickOnTopUpButton();
		expect(app().titleDisplayed()).toBe('Payment amount');		
		expect(app().nextButtonDisplayed()).toBe(true);
		expect(app().cancelButtonDisplayed()).toBe(true);
    });
